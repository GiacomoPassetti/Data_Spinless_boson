...
def H_Peier(g, J, Omega, h):
    ...
    H_bond_tebd_ev=Peier+Peier_hc+cav+ons_l+ons_r
    H_bond_tebd_odd=Peier+Peier_hc+cav  #This is the energetic term that will be used in the TEBD algorithm
    H_bond=Peier+Peier_hc+ons_l+ons_r  #This takes into account only the Peier term and is useful to evaluate the energy of the bonds
...
H_bond_tebd_ev=H_Peier(g, J, Omega, h)[0]
H_bond_tebd_odd=H_Peier(g, J, Omega, h)[1]
H_bond=H_Peier(g, J, Omega, h)[2]
...
with open('psi_ground_L_'+str(L)+'g_'+str(g)+'J_'+str(J)+'Omega_'+str(Omega)+'h_'+str(5)+'.pkl', 'wb') as f:
    pickle.dump(psi, f)

occupancy=psi.expectation_value('N')
np.save('occupancy_ground_L_'+str(L)+'g_'+str(g)+'J_'+str(J)+'Omega_'+str(Omega), occupancy)

with open("B_GS.txt", "a+") as file_object:
    ...
    file_object.write("g="+str(g)+"  NB="+str(occupancy[0]))
